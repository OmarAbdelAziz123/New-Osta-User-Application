...
class ChatScreenForUser extends StatefulWidget {
  const ChatScreenForUser({super.key, required this.data});

  final Map data;

  @override
  State<ChatScreenForUser> createState() => _ChatScreenForUserState();
}
  ...
  Future<void> stopRecording() async {
    ...
    Future.delayed(Duration.zero).then((value) async {
      await OffersOrdersCubit.get(context).inboxFunction(
          conversationId: widget.data['conversationId'].toString(),
          content: null,
          mediaList: [_filePath]);
    });
    setState(() {
    ...
    scrollController.addListener(() {
      if (scrollController.offset >=
        ...
        if (OffersOrdersCubit.get(context)
          ...
          // OffersOrdersCubit.get(context).getAllMessagesFunction(
          //     conversationId: widget.data['conversationId'].toString(), page: countPage, perPage: 10);
                                                          ...
                                                          if (contentErrors.isNotEmpty) {
                                                            for (var element in contentErrors) {
                                                              if (value[index].content == element) {
                                                                ...
                                                                inboxCubit.inboxFunction(conversationId: widget.data['conversationId'].toString(), content: element);
                                                                setState(() {});
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            //       GestureDetector(
                            //         onTap: () {
                            //           inboxCubit.makeOrderIsDoneFunc(conversationId: widget.data['conversationId'], paymentMethod: 'cash');
                            //         },
                            //         child: Container(
